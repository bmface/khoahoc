...
$sql = "SELECT * FROM tickets WHERE id = " . $id . " and user_id = " . $_SESSION['user_id'];
$stmt = $conn->prepare($sql);
$stmt->execute();
...
if (isset($_GET['close'])) {
    ...
    $sql = "UPDATE tickets SET status = 'User đã đóng' WHERE id = " . $id;
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (strpos($ticket['status'], 'User đã đóng') !== false) {
        // redirect to tickets
    }
    ...
    if (empty($message)) {
        ...
        // update status ticket
        $sql = "UPDATE tickets SET status = 'User trả lời' WHERE id = " . $id;
        $stmt = $conn->prepare($sql);
        $stmt->execute();
